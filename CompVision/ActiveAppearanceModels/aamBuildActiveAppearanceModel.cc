...
//  This program reads an appearance model and a set of appearance files and uses them to build an Active Appearance Model (AAM). The appearance files to be read are located in the directory defined by the option "-d" and are specified either by providing a list of file names (option "-l") or by providing the extension of the appearance files (option "-e"). In addition, it is possible to double the effective number of appearances used for building the AAM by considering mirror appearances of the appearance defined in the files. This is done by providing a mirror file (option "-m"). The mirror file consists of a list of pairs of indices which represent feature points in correspondence when carrying out the vertical symmetry (mirror effect). The AAM is trained by displacing successively each of the parameters from their ground truth value and measuring the resulting error. The option "-i" represents the half number of displacements carried out for each parameter. Typically a value of 1 (i.e. 2 displacements, 1 in the positive direction and 1 in the negative direction) is sufficient. The generation of an AAM can be very computer intensive especially if there is a large number of appearances available for training. The option "-pre" enables to save partial results for a set of files (before generation of the final AAM). This means that it is not necessary to process all the files simultaneously. Several jobs can be run in parallel, and the partial results can be ultimately merged by using the program "aamCombineActiveAppearanceModel". In the case where partial results must be computed (option "-pre"), the format for the output file is filename_i_n.abs, where 'i' and 'n' are integers. 'i' is the rank of the first appearance file to be considered from the list, 'n' is the number of files to be included starting from the 'i'th one. For more information type "aamBuildActiveAppearanceModel -help".
int main(int nargs,char **argv) {
  ...
  StringC ip = opt.String("am","/vol/vssp/vampire/people/jean-yves/appmodel/models/am_all_half1_25x22m98_98_95.abs","Input statistical model of appearance.");
  ...
  StringC op = opt.String("o","/vol/vssp/vampire/people/jean-yves/appmodel/models/am_all_half1_25x22m98_98_95/am_all_half1_25x22m98_98_95_1_100.abs","Output file for Active Appearance Model (AAM). In the case where partial results must be computed (option \"-pre\"), the format for the output file is filename_i_n.abs, where 'i' and 'n' are integers. 'i' is the rank of the first appearance file to be considered from the list, 'n' is the number of files to be included starting from the 'i'th one.");
  opt.Check();
