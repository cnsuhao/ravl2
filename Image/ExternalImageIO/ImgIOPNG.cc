...
static
void my_png_filein(png_structp png_ptr, png_bytep data, png_size_t length) {
  IStreamC &is = *((IStreamC *)png_get_io_ptr(png_ptr));
  assert(is.good());
...
static
void my_png_fileout(png_structp png_ptr, png_bytep data, png_size_t length) {
  OStreamC &os = *((OStreamC *)png_get_io_ptr(png_ptr));
  assert(os.good());
...
BooleanT DPIImageIOPNGBaseC::ReadHeader(const type_info &obj_type) {
  if (setjmp(png_jmpbuf(png_ptr))) 
    return false;
  ...
  if(obj_type == typeid(ByteRGBValueC)) {
    ...
    req_chan = 3;
  } else if(obj_type == typeid(ByteGreyValueT)) {
    ///////////////// ByteGreyValueT /////////////////////
    req_bit_depth = 8;
    req_color_type = PNG_COLOR_TYPE_GRAY;
    req_chan = 1;
  } else if(obj_type == typeid(GreyValueU16T)) {
    ///////////////// GreyValueU16T /////////////////////
    req_bit_depth = 16;
    req_color_type = PNG_COLOR_TYPE_GRAY;
    req_chan = 1;
  } else if(obj_type == typeid(ByteRGBXValueC)) {
    ///////////////// ByteRGBXValueC /////////////////////
    req_bit_depth = 8;
    req_color_type = PNG_COLOR_TYPE_RGB_ALPHA;
    req_chan = 4;
  } else if(obj_type == typeid(RGBValueU16C)) {
    ///////////////// RGBValueU16C /////////////////////
    req_bit_depth = 16;
    req_color_type = PNG_COLOR_TYPE_RGB;
    req_chan = 3;
...
BooleanT DPOImageIOPNGBaseC::WriteHeader(const type_info &obj_type,const ImageRectangleC &nimgRect) {
  imgRect = nimgRect;
  ...
  png_uint_32 width = imgRect.Cnum();
  png_uint_32 height = imgRect.Rnum();
...
DPIImagePNGByteRGBC::DPIImagePNGByteRGBC(FilenameC fn)
...
DPOImagePNGByteRGBC::DPOImagePNGByteRGBC(FilenameC fn)
...
DPIImagePNGByteGreyC::DPIImagePNGByteGreyC(FilenameC fn)
  : DPEntityC(*new DPIImagePNGBodyC<ByteGreyValueT>(fn))
{}
...
DPIImagePNGByteGreyC::DPIImagePNGByteGreyC(const IStreamC &strm)
  : DPEntityC(*new DPIImagePNGBodyC<ByteGreyValueT>(strm))
{}
...
DPOImagePNGByteGreyC::DPOImagePNGByteGreyC(FilenameC fn)
  : DPEntityC(*new DPOImagePNGBodyC<ByteGreyValueT>(fn))
{}
...
DPOImagePNGByteGreyC::DPOImagePNGByteGreyC(const OStreamC &strm)
  : DPEntityC(*new DPOImagePNGBodyC<ByteGreyValueT>(strm))
{}
