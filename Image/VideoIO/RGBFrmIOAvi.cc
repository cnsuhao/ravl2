...
namespace RavlImageN {
  ...
  DPISRGBAviFrameBodyC::DPISRGBAviFrameBodyC(const IStreamC &nStrm)
    : aviStrm(nStrm,false),
      currentframeNo(0) 
  {
    if(!aviStrm.Stream()) cerr << "DPISRGBAviFrameBodyC::DPISRGBAviFrameBodyC(IStreamC), Passed bad stream. \n";
  ...
  bool DPISRGBAviFrameBodyC::Seek(UIntT frameNo) 
  {
  ...
  bool DPISRGBAviFrameBodyC::DSeek(IntT frames) 
  {
  ...
  UIntT DPISRGBAviFrameBodyC::Tell() const { 
    return currentframeNo; 
  }
  
  //: Find the total size of the stream.
  UIntT DPISRGBAviFrameBodyC::Size() const 
  {
  ...
  ImageC<ByteRGBValueC> DPISRGBAviFrameBodyC::Get() 
  {
    ...
    ImageC<ByteRGBValueC> head(myframesize);
    Get(head);
    return head;
  }
  ...
  bool DPISRGBAviFrameBodyC::Get(ImageC<ByteRGBValueC> &head) 
  { 
    UIntT size;
    int h = (int)Abs(strf.biHeight);
    int w = (int)Abs(strf.biWidth);
    ...
    IndexRange2dC myframesize(h,w);
    if(head.Frame() != myframesize) head = ImageC<ByteRGBValueC>(myframesize);
  ...
  DPOSRGBAviFrameBodyC::DPOSRGBAviFrameBodyC(const OStreamC &nStrm)
    : currentframeNo(0),
      framesAdded(0)
  {
    if(!nStrm)
      cerr << "DPOSRGBAviFrameBodyC::DPOSRGBAviFrameBodyC(OStreamC), Passed bad stream. \n";
  ...
  bool DPOSRGBAviFrameBodyC::Seek(UIntT frameNo) 
  {
  ...
  bool DPOSRGBAviFrameBodyC::DSeek(IntT off) 
  {
  ...
  UIntT DPOSRGBAviFrameBodyC::Tell() const { 
    return currentframeNo; 
  }
  
  //: Find the total size of the stream.////////////////////////
  
  UIntT DPOSRGBAviFrameBodyC::Size() const { 
    return framesAdded; 
  }
  ...
  bool DPOSRGBAviFrameBodyC::Put(const ImageC<ByteRGBValueC> &Img)
  {
	...
	strf.biSizeImage=0;//set to 0 for uncompressed RGB bitmaps
  ...
  DPISRGBAviFrameC::DPISRGBAviFrameC(const StringC &fn)
    : DPEntityC(*new DPISRGBAviFrameBodyC(IStreamC(fn)))
  {}
  
  //: Constructor from filename.  
  
  DPOSRGBAviFrameC::DPOSRGBAviFrameC(const StringC &fn)
    : DPEntityC(*new DPOSRGBAviFrameBodyC(OStreamC(fn)))
  {}
}
