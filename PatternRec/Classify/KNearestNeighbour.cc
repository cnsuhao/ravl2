...
namespace RavlN {
  ...
  KNearestNeighbourBodyC::KNearestNeighbourBodyC(const DataSet2C<SampleVectorC,SampleLabelC> &ndata,
						 UIntT defK,
      ...
      data(ndata)
  {
    ...
    ONDEBUG(cerr << "KNearestNeighbourBodyC::KNearestNeighbourBodyC(), Data=" << data.Size() <<" Labels=" << labels << "\n");
  }
  
  //: Classify vector 'data' return the most likely label.

  UIntT KNearestNeighbourBodyC::Classify(const VectorC &data) const {
  ...
  VectorC KNearestNeighbourBodyC::Confidence(const VectorC &data) const {
    SArray1dC<Tuple2C<UIntT,RealT> > res = Search(data,defaultK);
    VectorC ret(Labels());
    ret.Fill(0);
  ...
  SArray1dC<Tuple2C<UIntT,RealT> > KNearestNeighbourBodyC::Search(const VectorC &ex,UIntT k) const {
    RavlAssert(k > 0);
    SArray1dC<Tuple2C<UIntT,RealT> > ret(k);
    ONDEBUG(cerr << "KNearestNeighbourBodyC::Search(), ex=" << ex << " k=" << k << "\n");
    ...
    *xit =  Tuple2C<UIntT,RealT>(it.Data2(),max);
    ...
    // Then do the rest.
    for(;xit && it;xit++,it++) {
      ONDEBUG(cerr << "KNearestNeighbourBodyC::Search(),1 Data1=" << it.Data1() << " ex=" << ex << "\n");
    ...
    for(;it;it++) {
      ONDEBUG(cerr << "KNearestNeighbourBodyC::Search(),2 Data1=" << it.Data1() << " ex=" << ex << "\n");
