...
///////////////////////////////////////////////////////////
//! rcsid="$Id: AVPlay.cc 4777 2005-01-18 14:28:18Z ees1wc $"
...
int doAVPlay(int nargs,char *args[]) 
{
  ...
  bool video = option.Boolean("novideo",true,"Display Simple Controls only. ");
  bool audio = option.Boolean("noaudio",true,"Display Simple Controls only. ");
  ...
  DPISPortC<AVFrameC > vidIn;
    ...
    if(audio)
    if(!OpenISequence(audioIn,infile,audioFormatIn,verb)) {
  ...
  DPIPlayControlC<AVFrameC> vpCtrl(vidIn,true,vidIn.Start(),vidIn.Size()); 
